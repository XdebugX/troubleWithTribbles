...
public class OptionsScreen implements Screen, InputProcessor {
	...
	private static int numTexts,p,selectBlobI,selection,numButtons,btnsTxt[],whatKind,whatText;
	...
	public void render(float delta) {
		...
		if (clicked) {
			...
			if (!GV.isAndroid) if (selection==2) doBack=true;
			if (GV.isAndroid) if (selection==2) {
	...
	private void sizes () {
		...
		numButtons=2;
		btnsTxt = new int [] {TH.ItxtSoundOn, TH.ItxtMusOn};
		
		if (GV.isAndroid) {
			numButtons=3;
			btnsTxt = new int [] {TH.ItxtSoundOn, TH.ItxtMusOn, TH.ItxtLogin};
		}
